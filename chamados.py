...
async def notificar_csu_integracao(chamado_dict: dict):
    ...
    try:
        ...
        # Conta quantos atendimentos CSU + Falha Integração foram abertos hoje
        hoje_inicio = datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0)
        hoje_fim    = datetime.now(timezone.utc).replace(hour=23, minute=59, second=59, microsecond=999999)
