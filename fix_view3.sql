CREATE OR REPLACE VIEW v_elo_tabelao AS
 WITH canal_lookup AS (
         ...
         SELECT DISTINCT ON (sac_entregas_rastreio.id_entrega) sac_entregas_rastreio.id_entrega,
            sac_entregas_rastreio.endereco_logradouro,
            sac_entregas_rastreio.endereco_numero,
            sac_entregas_rastreio.endereco_complemento,
            sac_entregas_rastreio.endereco_bairro,
            sac_entregas_rastreio.endereco_cep
           FROM sac_entregas_rastreio
          WHERE sac_entregas_rastreio.id_entrega IS NOT NULL
          ORDER BY sac_entregas_rastreio.id_entrega, sac_entregas_rastreio.id DESC
        )
 SELECT p.id_entrega AS entrega,
    ...
    -- Endereço: 1º sac_entregas_rastreio (endereço real de entrega),
    --           2º cadastro_terceiros quando CEP bate,
    --           3º NULL (não exibir rua errada)
    COALESCE(
        sd.endereco_logradouro,
        CASE
            WHEN ct.cep IS NOT NULL
             AND p.cep_destino IS NOT NULL
             AND regexp_replace(ct.cep::text, '[^0-9]', '', 'g') = regexp_replace(p.cep_destino::text, '[^0-9]', '', 'g')
            THEN ct.endereco
            ELSE NULL
        END
    ) AS endereco_rua,
   ...
   FROM (((((((((pedidos_dedup p
     ...
     LEFT JOIN sac_dedup sd ON (sd.id_entrega = p.id_entrega));
